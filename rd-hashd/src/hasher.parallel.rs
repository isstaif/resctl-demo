...
let start = Instant::now();
        ...
        let child1 = thread::spawn(|| {
            //println!("Child 1: Working...");
                let size = 500;
    //let matrix = DMatrix::from_fn(500, 500, |_, _| random::<f64>());
    //let result = compute_intensive_operation(&matrix);
        ...
        let child2 = thread::spawn(|| {
            //println!("Child 2: Working...");
                //let matrix = DMatrix::from_fn(350, 350, |_, _| random::<f64>());
    //let result = compute_intensive_operation(&matrix);
            for int in 0..34 { fib(int); }
        ...
        let result1 = child1.join().expect("Child 1 panicked");        
        let result2 = child2.join().expect("Child 2 panicked");
